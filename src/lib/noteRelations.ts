...
export async function attachNoteRelations(
  ...
  const catIds = Array.from(new Set(notes.map((n) => n.cat_id).filter((x): x is string => !!x)));
  const topicIds = Array.from(new Set(notes.map((n) => n.topic_id).filter((x): x is string => !!x)));
